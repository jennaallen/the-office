...
```{r}
 kable(head(entities), "html") %>% 
  kable_styling(bootstrap_options = c("striped", "hover"))
```

Here we see the entity identified and the entity type. We can join this table back to the original data by id to bring in our metadata such as speaker. From there we can again use tf-idf to see which entities were uniquely talked about by a given character.

```{r}
speaker_entities <- entities %>% 
  left_join(speaker, by = "id") %>% 
  filter(!(tolower(entity) %in% main_characters) & speaker %in% main_characters) %>% 
...
```{r}
kable(head(dependencies), "html") %>% 
  kable_styling(bootstrap_options = c("striped", "hover"))
```

This provides a lot of really useful information! We can see each word, lemma, word target, and lemma target. According to wikipedia "a lemma (plural lemmas or lemmata) is the canonical form, dictionary form, or citation form of a set of words. For example, run, runs, ran and running are forms of the same lexeme, with run as the lemma." This table provides the grammatical relationship between the word/lemma and the word_target/lemma_target. From this we can get common verb noun phrases, for example.

```{r}
dobj <- dependencies %>%
  filter(relation == "dobj") %>%
  select(id = id, verb = word, noun = word_target) %>%
  select(id, verb, noun) %>%
  count(verb = tolower(verb), noun = tolower(noun), sort = TRUE)
```

What is a direct object, you ask? The direct object of a verb is the thing being acted upon (i.e., the receiver of the action). From our earlier analysis, we say that characters commonly used the words god, time, #####put some other words here. Let's try to put a little more context around these words when they are used as the direct object. 

```{r}
bubble chart plot
```

We can also look at the noun subject relationships using the dependency table. The noun subject is the noun that performs the action of the verb in a sentence. 

```{r}
nsubj <- dependencies %>% 
  filter(relation == 'nsubj') %>%
  rename(verb = word, noun = word_target) %>% 
  count(noun, verb, sort = TRUE)

adj <- tokens %>% 
  mutate_at(vars(id), as.integer) %>% 
  filter(pos == 'JJ') %>%
  left_join(meta) %>% 
  count(adj = tolower(word), speaker, sort = TRUE)
